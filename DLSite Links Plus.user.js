...
// @match       *://gitgud.io/*
// @version     2.1.3.1
// @description Provide links and previews for RJ, RE, VJ, DMM, VG, and RG codes, ci-en blogs, steam, and vndb
...
class Chan {
  RegexList = [
    ...
    this.Steam = /(?:(?:https?[^>\s]+)?store\.steampowered\.com\/app\/|(?<![0-9a-zA-Z])s|^s)(\d{3,})(?:\/[^>\s]+)?/gi,
    this.VNDB = /(?:(?:(?:(?:[^>\s]+)?vndb.org\/([vr][1-9]\d*)|(?<![0-9a-zA-Z])v[1-9]\d*))(?!(?:\.\d|\w|-)))/g,
  ];
  ...
  async fetchHandler(src, anchor, site) {
    ...
    if (!success) {
      ...
      console.error(`dlsite links+: Failed to fetch ${src} from ${site}`)
    }
  }
    ...
    await GM.xmlHttpRequest({
      ...
      onload: function(response) {
        ...
        if (src.includes('article'))
          img = doc.querySelector('.file-player-image');
        if (!img)
  ...
  async fetchSteam(code, imgDiv) {
    ...
    await GM.xmlHttpRequest({
      ...
      onerror: function(response) { result = false; },
      onload: function(response) {
        const header = JSON.parse(response.response)[code]['data']['header_image'];
        if (!header)
          return false;
  ...
  createCien(match, creator, article) {
    const href = `https://ci-en.dlsite.com/creator/${creator}` + (article ? `/article/${article}` : '');
  ...
  createVNDB(match, code) {
    ...
    if (this.barCheck(bar, anchor))
      return anchor;
    this.addPreview(code, href, 'vndb');
    return anchor;
  }
  ...
  css() {
    ...
    .hgg2d__code__container .hgg2d__code {
      padding-right: 4px;
      width: 5rem;
      overflow: hidden;
  ...
  html() {
    ...
    this.hgg2d__settings.innerHTML = (`
      ...
      <div class="hgg2d__settings__main">
        ...
        <div>
          <input type="checkbox" ${this.settings.filterLimit ? 'checked' : ''} class="filterLimit">Ignore posts with
          <input type ='number' id='hgg2dPostFilterLimit' class="filterLimitCount" value = ${this.settings.filterLimitCount}></input> or more links.</input>
        </div>
      </div>
  ...
  work(node) {
    ...
    this.matchText(node, this.Steam, (match, code) => this.createSteam(match, code));
    this.matchText(node, this.VNDB, (match, code) => this.createVNDB(match, code));
